      ...
      for(m in 1:n.cov){
        for(rep in 1:n.rep){
          if(na.ind[l,m,rep]==FALSE){
            ll.theta[l,m,rep]=dcat(G.obs[l,m,rep],theta[samp.type[l],G.true[ID.curr[l],m],1:3],log=TRUE)
          }
        }
      }
    }
    ...
    for(l in 1:n.samples){
          ...
          for(m in 1:n.cov){
            for(rep in 1:n.rep){
              if(na.ind[l,m,rep]==FALSE){ #if observed
                G.probs[i]=G.probs[i]*theta[samp.type[l],G.true[i,m],G.obs[l,m,rep]]
              }
            }
          }
        }
      ...
      if(ID.curr[l]!=ID.cand[l]){ #skip if propose same ID
        ...
        for(m in 1:n.cov){
          for(rep in 1:n.rep){
            if(na.ind[l,m,rep]==FALSE){
              ll.theta.cand[l,m,rep]=dcat(G.obs[l,m,rep],theta[samp.type[l],G.true[ID.cand[l],m],1:3],log=TRUE)
            }else{
