GetDetectionRate <- nimbleFunction(
  run = function(s = double(1), beta0.lam0=double(0),beta1.lam0=double(0), 
                 effort=double(2),sigma=double(0), 
                 X=double(2), J=double(0), K=double(0),z=double(0)){ 
    returnType(double(2))
    if(z==0) return(matrix(0,J,K))
    if(z==1){
      ...
      for(j in 1:J){
        for(k in 1:K){
          lamd[j,k] <- lam0[j,k]*exp(-d2[j]/(2*sigma*sigma))
        }
      }
      return(lamd)
    }
...
dPoissonMatrix <- nimbleFunction(
  run = function(x = double(2), lam = double(2), K2D = double(2), z = double(0),
                 log = integer(0)) {
    returnType(double(0))
    if(z==0){
      ...
      logProb <- sum(dpois(x, lambda = lam*K2D, log = TRUE))
      return(logProb)
    }
  }
)
...
rPoissonMatrix <- nimbleFunction(
  run = function(n = integer(0),lam = double(2),K2D = double(2),z = double(0)) {
    returnType(double(2))
    J <- nimDim(lam)[1]
    K <- nimDim(lam)[2]
    out <- matrix(0,J,K)
...
IDSampler <- nimbleFunction(
  ...
  run = function() {
    #don't need to consider K2D in Poisson likelihood bc K2D[j,k]=1 for all detected samples.
    G.true <- model$G.true
    y.true <- model$y.true
    ID.curr <- model$ID
    z <- model$z
    ...
    ll.y <- array(0,dim=c(M,J,K)) #M x J most efficient here
    for(i in 1:M){
      if(z[i]==1){
        ll.y[i,1:J,1:K] <- dpois(y.true[i,1:J,1:K],lam[i,1:J,1:K],log=TRUE)
      }
    }
    ll.y.cand <- ll.y
    ...
    for(l in 1:n.samples){
      ...
      if(ID.curr[l]!=ID.cand[l]){ #skip if propose same ID
        ...
        ll.y.cand[swapped[2],this.j[l],this.k[l]] <- dpois(y.cand[swapped[2],this.j[l],this.k[l]],model$lam[swapped[2],this.j[l],this.k[l]],log=TRUE)
        #update ll.theta
        for(m in 1:n.cov){
          for(rep in 1:n.rep){
            if(na.ind[l,m,rep]==FALSE){
        ...
        #working with 1 i at a time bc nimble can't do math in 3D
        logProb.curr <- ll.y[swapped[1],this.j[l],this.k[l]] + 
          ll.y[swapped[2],this.j[l],this.k[l]] + 
          sum(ll.theta[l,,])
        logProb.cand <- ll.y.cand[swapped[1],this.j[l],this.k[l]] +
          ll.y.cand[swapped[2],this.j[l],this.k[l]] + 
          sum(ll.theta.cand[l,,])
        if(runif(1)<exp(logProb.cand-logProb.curr)*
           (backprob/propprob)*(focalbackprob/focalprob)){
          y.true[swapped[1],this.j[l],this.k[l]] <- y.cand[swapped[1],this.j[l],this.k[l]]
          ll.y[swapped[1],this.j[l],this.k[l]] <- ll.y.cand[swapped[1],this.j[l],this.k[l]]
          y.true[swapped[2],this.j[l],this.k[l]] <- y.cand[swapped[2],this.j[l],this.k[l]]
