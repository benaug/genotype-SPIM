...
init.data.negbin=function(data=NA,M=NA,inits=inits){
  library(abind)
  this.j <- data$this.j
  this.k <- data$this.k
  X<-as.matrix(data$X)
  ...
  lam0<- inits$lam0
  sigma<- inits$sigma
  if(!"theta.d"%in%names(inits))stop("Must provide an init for theta.d (dispersion)")
  ...
  for(j in 1:J){
    ...
    ll.y[,j]=dnbinom(y.true2D[,j],mu=lamd[,j]*z,size=theta.d*K1D[j],log=TRUE)
  }
