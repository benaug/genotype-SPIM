#This version allows for trap by occasion effort effects on detection probability
#Can remove effort if you just need occasion effects.
#Uses the 3D M x J x K capture history, so it is slower than 2D versions, increasingly so as K increases.
#if you want an effort effect with only 1 occasion, modify the 2D version, don't use this 3D version
#because nimble won't accept a K dimension of size 1 in the capture array, must use matrix.

library(coda)
library(nimble)
library(abind)
source("sim.genoSPIM.sampType.effort.R")
source("build.genos.R")
source("init.data.poisson.sampType.effort.R")
source("map.genos.R")
source("NimbleModel genoSPIM Poisson sampType Effort.R")
source("Nimble Functions genoSPIM Poisson sampType Effort.R")
source("sSampler.R")
...
K2D <- matrix(1,J,K) #indicator that trap j on occasion k has any effort (can't log 0 effort)
#put in some 0 effort trap-occasions
K2D[5,2] <- 0
K2D[5,8] <- 0
K2D[6,1] <- 0
effort <- log(effort) #log transform effort
...
data <- sim.genoSPIM.sampType.effort(N=N,beta0.lam0=beta0.lam0,
                              beta1.lam0=beta1.lam0,
                              effort=effort,K2D=K2D,
                              sigma=sigma,K=K,X=X,buff=buff,
                              obstype="poisson",
...
#need to set beta0.lam0 and beta1.lam0 so that starting logProb is finite. Test below.
#setting close to simulated values here, but not exactly at simulate values.
inits <- list(beta0.lam0=0,beta1.lam0=1,sigma=0.5,gammaMat=gammaMat,
              p.geno.het=p.geno.het.init,p.geno.hom=p.geno.hom.init) #plug in some ballpark estimates to initialize data
nimbuild <- init.data.poisson.sampType.effort(data=data,M=M,inits=inits)
...
J <- nrow(data$X)
constants <- list(M=M,J=J,K=K,n.samples=n.samples,n.cov=n.cov.use,n.rep=n.rep.use,
                ...
                samp.levels=samp.levels,effort=data$effort,K2D=data$K2D)

#supply data to nimble
Nimdata <- list(y.true=array(NA,dim=c(M,J,K)),G.obs=nimbuild$G.obs,
...
parameters <- c('psi','beta0.lam0','beta1.lam0',
                'sigma','N','n','p.geno.het','p.geno.hom','gammaMat')
...
conf$addSampler(target = paste0("y.true[1:",M,",1:",J,",1:",K,"]"),
                type = 'IDSampler',control = list(M=M,J=J,K=K,n.cov=n.cov.use,n.samples=n.samples,K2D=K2D,
                                                  n.rep=n.rep.use,this.j=data$this.j,this.k=data$this.k,
                                                  G.obs=data$G.obs,
