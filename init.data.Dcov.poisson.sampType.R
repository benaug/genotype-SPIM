...
init.data.Dcov.poisson.sampType <- function(data=NA,M=NA,inits=inits){
  library(abind)
  this.j <- data$this.j
  this.k <- data$this.k
  X <- as.matrix(data$X)
  ...
  constraints <- constraints <- matrix(1,nrow=n.samples,ncol=n.samples)
  for(i in 1:n.samples){
  ...
  if(n.cov==1){
    ...
    thetaArray <- abind(thetaArray,array(NA,dim=c(1,n.samp.levels,max(n.levels),max(n.levels))),along=1)
  }
