...
n.levels <- unlist(lapply(unique.genos,nrow)) #update n.levels in case some discarded
...
N <- 78
lam0 <- 0.25
theta.d <- 0.1 # negative binomial dispersion parameter. Lower is more dispersion
sigma <- 0.50
...
data <- sim.genoSPIM(N=N,lam0=lam0,theta.d=theta.d,sigma=sigma,K=K,X=X,buff=buff,#cap-recap parms
                  obstype="negbin",
                  n.cov=n.cov,pID=pID,n.rep=n.rep,
                  p.geno.hom=p.geno.hom,p.geno.het=p.geno.het,
                  gamma=gamma,IDcovs=IDcovs,ptype=ptype)

#look at true y x j x k counts to see if much signal of overdispersion.
#Generally harder to detect overdispersion than e.g., a negative binomial regression
#because activity centers not known and can be moved closer to traps with a lot of 
#counts to raise expected counts for that individual.
table(data$y)
...
inits <- list(lam0=1,sigma=1,theta.d=0.5,gammaMat=gammaMat,p.geno.het=c(0.95,0.025,0.025),p.geno.hom=c(0.95,0.05)) #plug in some ballpark estimates to initialize data
nimbuild <- init.data.negbin(data=data,M=M,inits=inits)
...
config.nodes <- c("p.geno.het","p.geno.hom","psi","sigma","lam0","theta.d","gammaMat","z")
